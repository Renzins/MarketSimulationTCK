...
│                               SoC-safety charts + Optimised-runs log)
...
│                               shared time-series window, per-tab Optimised-runs log)
│
...
├── tests_bess.py               21-test BESS suite (Python mirror of bess-engine.js):
│                               forecast/peak+trough ranks, core mechanics (efficiency,
│                               cost-basis, min-delta, whole-MW, red zones, dwell,
│                               must-fulfil, price-aware charging, DA buy-low,
│                               opportunistic divert), real-data invariants + frozen value
...
Two timing rules drive the design. **Imbalance is never known at bid time**, so
it is never a *chosen* close — the opportunistic divert always covers the DA
obligation on the intraday VWAP (the only price transactable ~30 min ahead);
imbalance only arises *passively* when a DA delivery physically falls short.
**Day-ahead can only be committed a day before**, so DA buy-low is a committed
forecast-trough leg, *not* a real-time charge source — mFRR (25 min) / aFRR /
intraday (30 min) are the real-time ones.
...
| **Opportunistic balancing override** | spike threshold (100) | if a balancing-up price beats the **intraday** buy-back by ≥ threshold, divert the battery's energy to the balancing spike and cover the DA obligation on intraday — the DA revenue is kept, so the gain is purely `balancing − intraday`. (Imbalance is never a chosen close; see the timing note above.) |
...
runs log as the wind-park page.
...
3. **Balancing bid lifecycle** — every offer by product, direction, price and MW:
   filled = cleared, hollow = rested, ✕ = **withdrawn before the 25-min gate**.
4. **Operations & SoC statistics** — avg charge/discharge block length, idle gap,
   cycles, throughput; plus the **SoC safety audit** (time at 0 %/100 %, DA
   shortfalls, and the `unfulfilled` counter that proves the must-fulfil
   guarantee held).
...
`python tests_bess.py` runs **21 tests** against a Python mirror of
`bess-engine.js`: lag-96 forecast + per-day peak/trough ranks; core mechanics on
synthetic hand-computed scenarios (efficiency, cost-basis + min-delta, power cap,
**whole-MW**, red zones, dwell, must-fulfil under random prices, price-aware
charging, day-ahead buy-low, opportunistic divert keeping DA + closing on
intraday); and real-data invariants (rev == Σ components, SoC ∈ [0, cap], whole-MW
volumes, day-type partition, strategy-off neutrality, both-splits static collapse).
The **frozen default-config revenue** is **2,979,642 €** over the full dataset — the browser `BessEngine`
matches this Python mirror **to the euro** (float32-vs-float64 drift ≈ 0.04 €),
establishing JS↔Python parity exactly as the wind-park frozen values do. Re-
freeze (`FROZEN_DEFAULT_REVENUE_EUR`) after any engine change or data refresh.
